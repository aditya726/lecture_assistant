    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-2xl font-bold text-foreground">Extract Topics</h1>
        </div>
        <p className="text-muted-foreground">Identify main topics and themes from your content</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <GlassCard className="p-8">
            <h2 className="text-xl font-semibold text-foreground mb-5">Topics</h2>
            {!result ? (
              ...
              <div className="space-y-6">
                {result.topics && result.topics.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-muted-foreground mb-2">Main Topics</h3>
                    <div className="flex flex-wrap gap-2">
                      {result.topics.map((topic, index) => (
                        <span key={index} className="px-4 py-2 rounded-lg border border-input bg-card text-foreground text-sm font-medium">
                          {topic}
                        </span>
                      ))}
                    </div>
                ...
                {result.subtopics && result.subtopics.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-muted-foreground mb-2">Subtopics</h3>
